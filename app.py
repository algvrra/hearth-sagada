...
def init_db():
    ...
    # Reset room auto-increment so IDs always start at 1 (prevents ever-growing IDs)
    c.execute("DELETE FROM sqlite_sequence WHERE name='rooms'")
    # NOTE: Bookings are intentionally NOT cleared on startup — real reservations must persist
    conn.commit()
    for r in rooms:
...
def booked_dates(rid):
    conn=get_db()
    bks=conn.execute("SELECT checkin,checkout FROM bookings WHERE room_id=? AND status IN ('confirmed','pending_deposit')",(rid,)).fetchall()
    conn.close(); dates=[]
    for b in bks:
...
if __name__=='__main__':
    init_db()
    print("\n✅ Casa Sagada running at: http://localhost:5000")
